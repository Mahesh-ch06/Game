...
export function GameCard({ game }: GameCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative flex aspect-[3/4] w-full flex-col overflow-hidden rounded-2xl sm:rounded-3xl liquid-glass-card border border-white/15 hover:border-white/35 transition-all duration-300 shadow-2xl"
    >
      ...
      <div className="absolute top-2.5 sm:top-4 left-2.5 sm:left-4 right-2.5 sm:right-4 flex items-center justify-between gap-1.5 z-10">
        <span className="inline-flex items-center gap-1 rounded-full bg-black/60 border border-white/20 px-2 sm:px-2.5 py-0.5 sm:py-1 text-[9px] sm:text-[10px] font-extrabold text-orange-400 backdrop-blur-xl shadow-sm">
          <Flame className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-orange-400" />
          {totalPlayed.toLocaleString()}
        </span>
        {isAvailable && (
          <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/20 border border-emerald-500/35 px-2 sm:px-2.5 py-0.5 sm:py-1 text-[9px] sm:text-[10px] font-extrabold text-emerald-400 backdrop-blur-xl shadow-sm">
      ...
      {/* Content */}
      <div className="relative z-10 flex h-full flex-col justify-end p-3 sm:p-6 md:p-7 pointer-events-none">
        <div className="mb-1 sm:mb-2 flex items-center gap-2 text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-primary">
          <span className="px-2 py-0.5 rounded-full bg-primary/15 border border-primary/30 text-sky-300 backdrop-blur-md">
            {game.genre}
          </span>
        </div>
        ...
        <div className="mt-2.5 sm:mt-4 pt-1">
          {isAvailable ? (
            <div className="flex w-full items-center justify-center gap-1.5 sm:gap-2 rounded-full liquid-glass-primary py-2 sm:py-2.5 text-[10px] sm:text-xs font-black text-white shadow-[0_0_20px_var(--color-primary-glow)] group-hover:scale-[1.02] transition-transform">
              <Play className="w-3 h-3 sm:w-3.5 sm:h-3.5 fill-white" />
              PLAY NOW
            </div>
          ) : (
            <div className="flex w-full items-center justify-center rounded-full bg-white/5 backdrop-blur-md border border-white/15 py-2 sm:py-2.5 text-[10px] sm:text-xs font-bold tracking-widest text-gray-300 uppercase">
              Coming Soon
            </div>
          )}
        </div>
